...
bool reachedDesiredState(const Point& currentPose, const Point& desiredState) {
    double distance = std::hypot(currentPose.x - desiredState.x, currentPose.y - desiredState.y);
    return distance < threshold; // Define a threshold for reaching the desired state
}
...
void run(RamseteController& ramseteController, 
    ...
    for (const auto& desiredState : motionProfilePoints) {
        while (true) {
            Point currentPose = Point(odometry.getX(), odometry.getY(), odometry.getHeading(), 0.0, 0.0);
