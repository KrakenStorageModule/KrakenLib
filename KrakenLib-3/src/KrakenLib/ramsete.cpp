...
// L, double max_velocity, double max_voltage)
//     : k1(k1), k2(k2), k3(k3), L(L), max_velocity(max_velocity),
//     max_voltage(max_voltage) {}

// void RamseteController::update(const Point& currentPose, const Point&
// desiredState) {
...
//     double omega_d = k3 * e_theta + k2 * v_d * sinc(e_theta) * e_y; //
//     Desired angular velocity

//     double u1 = -k1 * e_x; // Control input for velocity
//     double u2 = -k3 * e_theta - k2 * v_d * sinc(e_theta) * e_y; // Control
//     input for turning rate
...
// void RamseteController::applyMotorVoltages(pros::MotorGroup& leftDrive,
...
//     double v_d = std::hypot(desiredState.x, desiredState.y); // Desired
//     linear velocity double omega_d = desiredState.theta; // Desired angular
//     velocity
...
void RamseteController::update(const Point &currentPose,
  ...
  // Transform errors to the robot's local frame
  double e_x_local =
      cos(currentPose.theta) * e_x + sin(currentPose.theta) * e_y;
  double e_y_local =
      -sin(currentPose.theta) * e_x + cos(currentPose.theta) * e_y;

  // Compute desired linear and angular velocities
  double v_d =
      std::hypot(e_x_local, e_y_local); // Linear velocity towards the target
  double omega_d = desiredState.theta;  // Desired angular velocity (heading)

  // Ramsete control laws
  double v = v_d * cos(e_theta) + k1 * e_x_local;
  double omega = omega_d + k2 * e_y_local +
                 k3 * e_theta; // Adjust omega_d for heading error
