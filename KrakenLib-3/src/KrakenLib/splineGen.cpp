 ...
 *
 * This function calculates a quintic Hermite spline through the waypoints, using
 * automated tension to create a smooth path. The result is a sequence of points
 * that make up the spline, which can be used to generate a path for a robot to
 * follow.
 */
...
//             tension = std::max(0.1, std::min(tension / 10.0, 1.0)); // Clamp tension between 0.1 and 1.0
//         }
...
//             double h0 = 1 - 10 * t * t * t + 15 * t * t * t * t - 6 * t * t * t * t * t; // h_0(t)
//             double h1 = t - 6 * t * t * t + 8 * t * t * t * t - 3 * t * t * t * t * t; // h_1(t)
//             double h2 = t * t * (2 * t - 3) + 9 * t * t * t * t - 2 * t * t * t * t * t; // h_2(t)
//             double h3 = 10 * t * t * t - 15 * t * t * t * t + 6 * t * t * t * t * t; // h_3(t)
